...
:loading
rem 5s fake-loading animation: 10 lines + bar, single-line CR refresh
...
for %%P in ("%PHRASES:;=","%") do (
    ...
    set /a FILLED=STEP*2
    set /a PCT=STEP*10
    set /a REST=20-FILLED
    set "BARF="
    for /l %%i in (1,1,!FILLED!) do set "BARF=!BARF!#"
    set "BARE="
    for /l %%i in (1,1,!REST!) do set "BARE=!BARE!-"
    <nul set /p "=!CR!%%~P [!BARF!!BARE!] !PCT!%%   "
    ping -n 1 -w 450 192.0.2.1 >nul
)
<nul set /p "=!CR!Loaded. [####################] 100%%"
echo.
endlocal
exit /b 0
