...
class _Palette:
    def __init__(self, enabled: bool):
        ...
        self.unicode = _encodable(*_DECOR)
    ...
    def status(self, status: str, icon: bool = False) -> str:
        text = self.icon(status) if icon else status
...
def _all_check_defs() -> list[dict]:
    ...
    defs: list[dict] = []
    try:
        defs.extend(get_core().list_checks())
    except (CoreNotAvailable, RuntimeError) as e:
        typer.secho(
            f"提示: 未读取到核心检查项（{e}），下面只列出 Python 侧检查",
            fg=typer.colors.YELLOW, err=True,
        )
    defs.extend(pychecks.py_check_defs())
    defs.sort(key=lambda d: (d.get("category", ""), d.get("id", "")))
    return defs


def _print_report(report: dict, pal: _Palette, expanded: set[str], expand_all: bool) -> str:
    """打印分类折叠报告，返回 verdict 状态（good / issues / error）。"""
    ...
    for cat in CATEGORIES:
        ...
        _echo_safe(pal.paint(f"{pal.bullet()} {cat}", "1") + f"  {head}")
        if expand_all or cat in expanded:
            for r in rows:
                ...
                for d in r.get("detail", []):
                    _echo_safe(f"      {pal.dot()} {d}")
                if r.get("hint"):
                    _echo_safe(pal.paint(f"      {pal.arrow()} 建议: {r['hint']}", "33"))

    verdict_state, problems = verdict(report)
    _echo_safe("")
    _echo_safe(pal.paint(pal.rule(), "1"))
    counts = report["summary"]["counts"]
    _echo_safe("统计: " + "  ".join(f"{pal.status(s, icon=True)}{n}" for s, n in counts.items()))
    if verdict_state == "good":
        _echo_safe(pal.paint(f"{pal.icon('ok')} 未发现需要处理的问题", "32"))
    elif verdict_state == "error":
        _echo_safe(pal.paint(f"{pal.icon('fail')} 诊断引擎异常: {report.get('error')}", "31"))
    else:
        _echo_safe(pal.paint(f"发现 {len(problems)} 个需要关注的问题:", "1"))
        for n, r in enumerate(problems, 1):
            _echo_safe(f"  {n}. {pal.status(r['status'], icon=True)} [{r['id']}] {r['title']}")
            if r.get("hint"):
                _echo_safe(pal.paint(f"     {pal.arrow()} {r['hint']}", "33"))
...
def _invoke_run_with_defaults(ctx: typer.Context) -> None:
    ...
    - `ctx.invoke(click_command)`：本环境 click 版本的 `Context.invoke` 是裸转发
      `callback(*args, **kwargs)`，传命令对象等于调用 `command()` → `main()` →
...
def run(
    ...
    known_tools = {d["id"] for d in core_obj.list_checks() if d.get("category") == "toolchains"}
    if known_tools:
    ...
    verdict_state = _print_report(report, pal, set(expand), expand_all)
